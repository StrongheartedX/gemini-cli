        ...
        typeof inputSchema === 'object' && inputSchema !== null
          ? Object.entries(
              ...
              .map(([key, prop]) => {
                ...
                const type =
                  (prop as Record<string, unknown>)['type'] || 'unknown';
                return `'${key}' (${isRequired ? 'required' : 'optional'} ${type})`;
              })
              .join(', ')
          : 'unknown';
      throw new Error(
        `Invalid arguments for agent '${definition.name}': ${validationError}. Expected inputs: ${expectedInputs}.`,
      );
    }
